...
// 掘金粉丝数据接口基础URL
const JUEJIN_API_URL = 'https://api.juejin.cn/user_api/v1/follow/followers';
...
export async function GET(request: Request) {
  try {
    ...
    if (!data || data.err_no !== 0) {
      throw new Error('获取掘金粉丝数据失败');
    }

    // 格式化粉丝数据
    const fans = data.data.map((item: any) => ({
      id: item.user_id,
      name: item.user_name,
    ...
    console.error('获取掘金粉丝数据出错:', error);
    return NextResponse.json(
      { error: '获取粉丝数据失败', message: (error as Error).message },
      { status: 500 }
    );
  }
}
