...
export function BandHobby({ preview = false }: BandHobbyProps) {
  ...
  const [bands] = React.useState<BandInfo[]>([
    {
      ...
      comment: "场次时常爆满，很火",
      scene:
        "在晚上的某个时间节点，你骑着车子，吹着风…… 表达夜生活的喧嚣 深夜的晚风 孤独又快乐",
    },
    {
      id: 2,
      name: "棱镜乐队",
  ...
  return (
    ...
    >
      ...
      <ul className="space-y-2" data-oid="0eglipa">
        {displayBands.length === 0 ? (
          <li className="py-2 text-muted-foreground/80" data-oid="eege1ku">
            暂无记录
          </li>
        ) : (
          displayBands.map((band) => (
            <li
              ...
              data-oid="vwnu1cc"
            >
              <div className="space-y-2.5" data-oid="6kwo4fp">
                <h3
                ...
                <div
                  className="space-y-1.5 text-muted-foreground/80"
                  data-oid="_vou0ui"
                >
                  <p className="flex items-baseline" data-oid="vo:g4s9">
                    <span className="w-20 shrink-0" data-oid="l7m:mhq">
                      🎵 作品
                    </span>
                    <span className="text-foreground/90" data-oid="xoq2wsc">
                      {band.favoriteWork}
                    </span>
                  </p>
                  {band.comment && (
                    <p className="flex items-baseline" data-oid="5zhv6s9">
                      <span className="w-20 shrink-0" data-oid="hyrxkxs">
                        💬 简评
                      </span>
                      <span data-oid="h:f16m5">{band.comment}</span>
                    </p>
                  )}
                  {band.scene && (
                    <p className="flex items-baseline" data-oid="0kcnj__">
                      <span className="w-20 shrink-0" data-oid=".4_v1eh">
                        🌆 场景
                      </span>
                      <span data-oid="tra79i-">{band.scene}</span>
                    </p>
                  )}
                </div>
              </div>
