...
export function CodeStats({ username }: CodeStatsProps) {
  ...
  const [dataType, setDataType] = React.useState<'additions' | 'deletions' | 'total'>('total');
          ...
          try {
            ...
            for (const commit of commits) {
              ...
              try {
                ...
                const current = statsMap.get(date) || { additions: 0, deletions: 0, total: 0, date };
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: dataType === 'additions' ? '新增代码行数' :
               dataType === 'deletions' ? '删除代码行数' : '净增代码行数',
